        ...
        with db.engine.begin() as connection:
            result = connection.execute(
                sqlalchemy.text(
                    ...
                    SELECT
                    coalesce(inbound.from_user, outbound.to_user) as user,
                    (
                        coalesce(inbound.amount, 0) - coalesce(outbound.amount, 0)
                    ) as amount
                    FROM
                    inbound full join outbound on inbound.from_user = outbound.to_user
                    '''
                ),
                {
                    'uid1': user_id
